<template>
  <webview nodeintegration  :id="id" :src="src"></webview>
</template>
...
export default {
  ...
  mounted() {
    ...
    document.getElementById(this.id)
        .contentWindow.addEventListener('message', this.onMessage)
  },
  beforeDestroy() {
    document.getElementById(this.id)
        .contentWindow.removeEventListener('message', this.onMessage)
  }
}
</script>
