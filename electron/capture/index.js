...
const winURL = `file://${path.resolve(__dirname, '../../asset/capture/index.html')}`;
...
function createWindow() {
    ...
    if (islinux && displays.length > 1) {
        dialog.showMessageBox({
            ...
            cutFun();
            global.MAIN_WINDOW.webContents.send('has-click-cut', false);
...
function cutFun() {
    globalShortcut.register(cutKey, () => {
        ...
        if (global.MAIN_WINDOW.isFullScreen()) {
            global.MAIN_WINDOW.setFullScreen(false);
        }
        // 截图时隐藏IM窗口
        if (isCutHideWindows) {
